...
def extract_camb_params(block, config, more_config):
    ...
    # 🔹 Forzar a CAMB a usar el A_s corregido
    if block.has_value(cosmo, 'sigma_8'):
        print(f"⚠️ Eliminando sigma_8 viejo: {block[cosmo, 'sigma_8']}")
        del block[cosmo, 'sigma_8']  # Eliminar sigma_8 para que CAMB lo recalcule desde A_s
        
    # Asegurar que A_s se pasa correctamente
# Si A_s no está en el datablock, usa el default
    if not block.has_value(cosmo, 'A_s'):
        print("⚠️ CAMB no recibió A_s de sigma8_rescale.py. Usando default.") 
        block[cosmo, 'A_s'] = DEFAULT_A_S
    else:
        print(f"✅ CAMB recibió A_s corregido: {block[cosmo, 'A_s']}")
...
def save_matter_power(r, block, more_config):
    ...
    for transfer_type in more_config['power_spectra']:
        ...
        #k = np.linspace(kcalc[0], kmax_power, more_config['nk'])
...
def execute(block, config):
    ...
    # 🔹 Verificar si sigma_8 está en el datablock, si no, asignarlo antes de recompute_for_sigma8()
           if not block.has_value(cosmo, 'sigma_8'):
               block[cosmo, 'sigma_8'] = block[cosmo, 'sigma8_input']
    
           r = recompute_for_sigma8(r, block, config, more_config)  #Ahora se ejecutará siempre
